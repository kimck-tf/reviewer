...
- 7개 SA를 동시 dispatch:
  - **카테고리 SA (6개, 슬라이드 단위 미시 검토)** — 결과는 배열 → raw `findings[]`로 concat
...
- 6개 카테고리 SA 응답을 concat하여 raw `findings[]` 생성
- raw `findings[]`를 `review_ws/findings_raw.json`에 저장 (스키마: `findings[]`만, 각 finding은 단수 `category` 필드)
- document SA 응답(객체 1개)은 메모리에 보관 (Step 3.5에서 합산)

### Step 3.5: Merger SA dispatch (raw findings 통합)
같은 원문·같은 대상에 대한 다중 카테고리 중복 지적을 단일 finding으로 종합.

- 1개 SA dispatch:
  - `report-reviewer-merger` (ID prefix `F`, 출력은 통합된 `findings[]`)
- 입력: `findings_raw.json` 경로 + `extracted.json` 경로
- 처리:
  - 같은 `quoted_text` 또는 같은 `(slide_index, shape_id)` + 의미적 동일 대상 → 단일 finding으로 재작성
  - 단일 카테고리에만 속한 finding은 `categories:[1개]·source_finding_ids:[1개]`로 통과
- 결과: 통합 `findings[]` (각 finding은 `categories[]`·`source_finding_ids[]` 필드 포함)
- 심각도·카테고리 통계는 통합 후 기준으로 집계
- 최종 `review_ws/findings.json` 저장 (스키마: `summary`, `findings[]`(통합), `document_review`)
...
- **Subagent 실패**: 1회 재시도, 그래도 실패 시 placeholder 결과로 채우고 리포트에 명시 (카테고리 SA 실패 시 빈 배열 `[]`, document SA 실패 시 `document_review` 키 생략, Merger SA 실패 시 raw findings를 그대로 사용하되 각 finding을 `categories:[원래]·source_finding_ids:[원래id]`로 minimal 변환하여 다운스트림 호환 유지)

## 옵션 처리
- `--resume`: extracted.json 있으면 Step 1 건너뜀, slide_summaries.json 있으면 Step 2도 건너뜀, findings_raw.json 있으면 Step 3도 건너뜀
- `--rerun-stage2`: extracted.json + slide_summaries.json 사용, Step 3부터 (Merger 포함)
- `--rerender`: findings.json 사용, Step 4만
- `--rerun-merge`: findings_raw.json 사용, Step 3.5(Merger)부터 (raw 검토 결과는 그대로, 통합만 재실행)
